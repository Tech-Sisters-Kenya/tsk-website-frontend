...
export function SimpleEditor({ editor }: { editor: Editor | null }) {
  const { isMobile, availableHeight } = useIsMobile();
  ...
  const getToolbarPosition = React.useCallback(() => {
    if (!isMobile || !editorContainerRef.current) return {};

    const containerRect = editorContainerRef.current.getBoundingClientRect();
    const containerBottom = containerRect.bottom;
    const viewportHeight = availableHeight || windowSize.height;

    // Calculate position relative to the editor container
    const relativeBottom = Math.max(0, viewportHeight - containerBottom);

    return {
      bottom: `${relativeBottom}px`,
    };
  }, [isMobile, availableHeight, windowSize.height]);
